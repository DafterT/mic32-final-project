## Практический пример Лекции 8
Исходный код показывает простейший механизм отправки команд с управляющего устройства (компа) на микроконтроллер, который отрабатывает получаемые команды.
Амур подключён через преобразователь USB-UART по Type C интерфейсу. Скорость работы UART - 115200, 1стоп, контроля чётности нет.
Через picocom / Bray Terminal можно отправить три команды: A, B, C. Контроллер, получая их, будет выводить на LCD дисплей строку, целые числа или числа с плавающей точкой, соответственно.

Программа работает с SPI флешки, тк не помещается в основную память. 
Драйвер для работы с дисплеем находится в lcd_driver.h/.c

## Домашнее задание

Подключить на макетную плату датчик температуры и влажности DHT11 и отобразить данные на дисплее.
Можно использовать дисплей LCD1602, драйвер для которого у тебя уже есть, или подключить самостоятельно дисплей из набора, который работает по шине SPI.

Ознакомится с даташитом на датчик можно по ссылке ниже. Код можно написать самостоятельно или же портировать из имеющихся открытых библиотек в собственный модуль на языке Си.
 * [Datasheet DHT11](https://static.chipdip.ru/lib/185/DOC001185323.pdf)
 * [Library DHT11](https://github.com/dhrubasaha08/DHT11)
